...
Read the safety note before you apply power the
...
Flux the pads heavily, then tack one corner pin to hold the chip square.
Kapton tape across the body helps hold it while you tack.
Tack the diagonal corner. Now it's located; both hands are free.
Drag-solder each row, or deliberately blob all pins and wick the excess
...
Inspect under magnification and buzz every adjacent pin pair for bridges,
...
RAW_SCAN     = True    # print raw (row,col) of every press, ignore keymap
XT_CONNECTED = False   # scan/print only; do NOT drive the XT lines
```
Step 1 — verify the matrix over USB (nothing connected to the Compaq)
Set `RAW_SCAN = True`, `XT_CONNECTED = False`. Open the serial console and press
...
the drive/sense sense noted in `scan_and_emit()`.
...
final confidence pass you can leave `DEBUG = True` with `XT_CONNECTED = True` —
...
---
Troubleshooting
Keys scan correctly (RAW_SCAN looks right) but the Compaq gets garbage, and
...
`xt_ready()` is returning false, so nothing transmits and the send queue backs
up. Check that RESET (input) reads high with the machine running, and that
CLOCK_SENSE idles high. A stuck-low reset or clock-sense blocks all sends.
Keys lag / need to be held.
Debounce too aggressive for your switches, or the scan interval is stretched.
Lower `DEBOUNCE_SCANS` (try 1) and reduce the per-row settle. If it still drags
